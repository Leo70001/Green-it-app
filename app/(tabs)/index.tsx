...
// import * as Updates from 'expo-updates'; // Keep this import if you still want the hard reload as an option
...
export default function Index() {
  ...
  // Shared values for map animation on mount (or soft reset)
  const mapOpacity = useSharedValue(0);
  const mapTranslateY = useSharedValue(20);

  // --- NEW: resetScreen function ---
  const resetScreen = useCallback(() => {
    // 1. Reset active continent
    setActiveContinent(null);

    // 2. Reset all continent fill animations
    (Object.keys(animValues) as Continent[]).forEach((key) => {
      animValues[key].value = withTiming(0, { duration: 500 }); // Animate back to default color
    });

    // 3. Reset label animations
    labelOpacity.value = withTiming(0, { duration: 300 });
    labelTranslateY.value = withTiming(10, { duration: 300 });

    // 4. (Optional) Re-run initial map animation if you want it to fade in again on reset
    mapOpacity.value = withTiming(0, { duration: 0 }); // Instantly set to 0
    mapTranslateY.value = withTiming(20, { duration: 0 }); // Instantly set to 20
    mapOpacity.value = withTiming(1, { duration: 1000 }); // Fade in
    mapTranslateY.value = withTiming(0, { duration: 1000 }); // Translate up
  }, [animValues, labelOpacity, labelTranslateY, mapOpacity, mapTranslateY]); // Dependencies for useCallback

  // --- MODIFIED: onRefresh to use resetScreen ---
  const onRefresh = useCallback(() => {
    setRefreshing(true); // Start showing the refresh indicator

    setTimeout(() => {
      resetScreen(); // Call the new reset function
      setRefreshing(false); // Hide the refresh indicator
    }, 1000); // Simulate a short delay for refresh
  }, [resetScreen]); // Dependency for useCallback
  ...
  // On mount animate map (now also called by resetScreen)
  useEffect(() => {
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={onRefresh} // Calls the soft reset function
            tintColor="#fff" // Color of the refresh indicator
            title="Rafraîchissement..." // Text shown on iOS
